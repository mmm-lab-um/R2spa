...
| F6 | mirt `psi` = the estimated factor covariance. The **single-group** path is handled: `get_fs.SingleGroupClass()` uses `mirt_full_cov()` (reads `COV_ij` / factor variances) as `psi` — **resolved 2026-08-22 (fc16f81)** (supersedes the `psi = diag(q)` wording in #15 / PLAN 08 for the SG path). Remaining: the **multi-group** path must use the **group-specific** covariance (per-group, not one shared matrix) — see F7. | plan `archive/PLAN_08_mirt_fs.md` | SG resolved 2026-08-22 (fc16f81); MG deferred (F7) |
| F7 | multi-group mirt `get_fs()` (`MultipleGroupClass`) — implement the `get_fs.MultipleGroupClass()` stub (currently errors "fit or extract a single group first"): per-group `psi` via `mirt_full_cov(mirt::extract.group(object, g))`; per-observation scores/SEs + `return.acov` scored across the whole fit (group from `mirt::extract.mirt(object, "group")`, global row order); `format = "unified"` = a `group` column + per-group `psi`/`alpha`; reuse the SG per-row `mirt_per_obs` list attrs so `fs_indiv()` keeps its one-block-per-row contract. Design: `archive/PLAN_10_multigroup_mirt.md`. | plan `archive/PLAN_10_multigroup_mirt.md` | planned (PLAN 10) |
