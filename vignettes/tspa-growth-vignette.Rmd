...
vignette: >
  %\VignetteIndexEntry{Linear Growth Modeling with Two-Stage Path Analysis}
...
saveRDS(eclsk, file = "eclsk.rds")
```

```{r, include = FALSE}
eclsk <- readRDS("eclsk.rds")
```
...
For objects extracted from `get_fs()`, one can omit the `fsT`, `fsL`, and `fsb` arguments, as the `tspa()` function will automatically extract the necessary information from the `fs_dat` object.
