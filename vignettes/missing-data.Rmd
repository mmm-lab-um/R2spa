...
# carrying that case's loadings / error variances (the exact, non-pooled
# correction rather than `tspa()`'s `reduce = ` pooling).
cross_load <- matrix(c(
...
# loadings / error variances, so drop their dem65 score (OpenMx's raw-data
# FIML then fits those rows as usual).
...
```{r}
# Obtain tidy-ed factor scores data (Bartlett weights); the same non-
# identifiable rows get their dem65 score dropped
